...
class Waveform:
    ...
    def __init__(   self, 
        ...
        htype.check_type( t0, float, np.float64,
                            exception_message=htype.generate_exception_message( "Waveform.__init__", 
                                                                                10001))
        htype.check_type( signal, np.ndarray,
                            exception_message=htype.generate_exception_message( "Waveform.__init__", 
                                                                                10002))     
        if np.ndim(signal)!=1:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "Waveform.__init__", 
                                                                                    10003))
        htype.check_type( signal[0], np.float64,
                            exception_message=htype.generate_exception_message( "Waveform.__init__", 
                                                                                10004))     
                            
        if t_step is None and time is None:
            raise cuex.InsufficientParameters(htype.generate_exception_message( "Waveform.__init__", 
                                                                                10005))
        elif t_step is not None:
            # If both, time and t_step are defined, t_step configuration is used by default
            htype.check_type( t_step, float, np.float64,
                                exception_message=htype.generate_exception_message( "Waveform.__init__", 
                                                                                    10006))
            if t_step<=0.:
                raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "Waveform.__init__", 
            ...
            htype.check_type( time, np.ndarray,
                                exception_message=htype.generate_exception_message( "Waveform.__init__", 
                                                                                    10008))     
            if np.ndim(time)!=1:
                raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "Waveform.__init__", 
                                                                                        10009))
            htype.check_type( time[0], np.float64,
                                exception_message=htype.generate_exception_message( "Waveform.__init__", 
                                                                                    10010))     
        ...
        if infer_t0 is not None:
            htype.check_type( infer_t0, bool,
                                exception_message=htype.generate_exception_message( "Waveform.__init__", 
                                                                                    10012))
            
        self.__signs = ListsRKD(    Waveform.nonstackable_aks+Waveform.stackable_aks,                           #potential_keys
                                    is_subtyped=True,                                                           #is_subtyped=False
                                    values_subtypes=[str, str, str, float, float, float, float, float, float])  #values_subtypes=None
        if signs is not None:
            htype.check_type( signs,  dict,
                                exception_message=htype.generate_exception_message(   "Waveform.__init__", 
                                                                                        10013))
            for key in signs.keys():
                                    #(key, value, overwrite)
                    self.Signs =    (key, signs[key], True) # The self.__signs setter (Waveform.Signs.setter) will handle
        ...
        if fUseTStep:
            ...
            self.__time     = np.linspace(  0, 
                                            ((self.__npoints-1)*t_step), 
                                            self.__npoints)
            self.__t0       = t0
        else:
            ...
            if infer_t0 and self.__time[0]!=0.:
                self.__t0   = self.__time[0]
            else:
                self.__t0   = t0
            self.__time     -= self.__time[0]

        self.__integral = None  # This attribute is not computed by default.
                                # It is only computed by self.integrate().

        # Compute baseline by default
        self.compute_first_peak_baseline(signal_fraction_for_median_cutoff=signal_fraction_for_median_cutoff)   
        return
    
    # Class variables
    nonstackable_aks = ['time_unit',        #aks stands for Allowed KeyS
                        'signal_magnitude', 
                        'signal_unit',  
                        'integration_ll', 
                        'integration_ul',
    ...
    def compute_first_peak_baseline(self, signal_fraction_for_median_cutoff = 0.2):
        ...
        htype.check_type(   signal_fraction_for_median_cutoff, float, np.float64,
                            exception_message=htype.generate_exception_message( "Waveform.compute_first_peak_baseline",
                                                                                20001))
        if signal_fraction_for_median_cutoff<0. or signal_fraction_for_median_cutoff>1.:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "Waveform.compute_first_peak_baseline",
                                                                                    20002))
        cutoff_idx = round(signal_fraction_for_median_cutoff*len(self.__signal))
        signal_chunk = Waveform.filter_infs_and_nans(   self.__signal[:cutoff_idx], 
                                                        get_mask=False)  

        if len(signal_chunk)==0:    # Happens if the signal_fraction_for_median_cutoff 
    ...
    def plot(self,  ax,
        ...
        htype.check_type(   plot_peaks, bool,
                            exception_message=htype.generate_exception_message( "Waveform.plot", 
                                                                                30001))
        ...
        if 'signal_magnitude' in self.__signs.keys():
            if 'signal_unit' in self.__signs.keys():
                ax.set_ylabel(f"{self.__signs['signal_magnitude'][0]} ({self.__signs['signal_unit'][0]})")
            else:
                ax.set_ylabel(f"{self.__signs['signal_magnitude'][0]} (a.u.)")
        if 'integration_ll' in self.__signs.keys():
            ax.axvline( x=self.__signs['integration_ll'], 
                        linestyle='-', 
                        linewidth=0.5, 
                        color='red', 
                        label="Int. LL")
        if 'integration_ul' in self.__signs.keys():
            ax.axvline( x=self.__signs['integration_ul'], 
                        linestyle='-', 
                        linewidth=0.5, 
                        color='red', 
                        label="Int. UL")
        if 'first_peak_baseline' in self.__signs.keys():
            ax.axhline( y=self.__signs['first_peak_baseline'], 
                        linestyle='-', 
                        linewidth=0.5, 
                        color='blue', 
                        label="T.P. Baseline")
        if 'median_cutoff' in self.__signs.keys():
            ax.axvline( x=self.__signs['median_cutoff'], 
                        linestyle='-', 
                        linewidth=0.5, 
                        color='blue', 
                        label="Median cutoff")
    ...
    def Signs(self, pack):
        ...
        htype.check_type(   pack, tuple, list,
                            exception_message=htype.generate_exception_message( "Waveform.Signs.setter",
                                                                                40001))
        if len(pack)!=3:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "Waveform.Signs.setter",
                                                                                    40002))
        key, value, overwrite = pack    # Unpack the arguments
        htype.check_type(   value, list,
                            exception_message=htype.generate_exception_message( "Waveform.Signs.setter",
                                                                                40003))
        ...
        if key in ['time_unit', 'signal_magnitude', 'signal_unit']:
            for element in value:
                htype.check_type(   element, str,
                                    exception_message=htype.generate_exception_message( "Waveform.Signs.setter",
                                                                                        40005))
            fIsStackable=False
        elif key in ['integration_ll', 'integration_ul', 'first_peak_baseline', 'median_cutoff']:
            for element in value:
                htype.check_type(   element, float, np.float64,
                                    exception_message=htype.generate_exception_message( "Waveform.Signs.setter",
                                                                                        40006))
            fIsStackable=False
            ...
            return  # If the key is not recognised, no addition to self.__signs is done
        ...
        if not overwrite and not fIsStackable:
            if key in self.Signs.keys():
                return  # This is the only case in which
                        # nothing is added to self.Signs 
        self.Signs.__setitem__( key, 
                                TypedList(init_members=value), 
                                append=not overwrite and fIsStackable)
        return
    ...
    def integrate(self, integration_lower_lim=None, integration_upper_lim=None):
        ...
        i_low, i_up = Waveform.adjust_integration_limits(   self.__time, 
        ...
        self.__integral = np.trapz(self.__signal[i_low:i_up+1]-self.Signs['first_peak_baseline'], x=self.__time[i_low:i_up+1])
        return self.__integral
    
    def find_beginning_of_rise(self,    tolerance=0.05,
        ...
        htype.check_type(   tolerance, float, np.float64,
                            exception_message=htype.generate_exception_message( "Waveform.find_beginning_of_rise", 
                                                                                50001))
        if tolerance<=0.0 or tolerance>=1.0:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "Waveform.find_beginning_of_rise", 
                                                                                    50002))
        htype.check_type(   return_iterator, bool,
                            exception_message=htype.generate_exception_message( "Waveform.find_beginning_of_rise", 
                                                                                50003))
        
        signal_maximum = np.max(Waveform.filter_infs_and_nans(self.__signal, get_mask=False))
        
        aux = signal_maximum-self.__signs['first_peak_baseline'][0]

        if aux<=0.0:    # This is a cross-check. Up to the computation of 
        ...
        if result_iterator==-1:     # If the first point of self.__signal exceeded
            result_iterator = 0     # the threshold, then result_iterator is -1. 
                                    # In this case, return 0 instead of of -1.
            
        if result_iterator==-2:     # If no point surpassing threshold
            ...
            raise cuex.MalFunction(htype.generate_exception_message("Waveform.find_rise_idx", 
                                                                    50005,
    ...
    def adjust_integration_limits(timearray, ill_candidate=None, iul_candidate=None):
        ...
        htype.check_type( timearray, np.ndarray,
                            exception_message=htype.generate_exception_message( "Waveform.adjust_integration_limits", 
                                                                                60001))     
        if np.ndim(timearray)!=1:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "Waveform.adjust_integration_limits", 
                                                                                    60002))
        htype.check_type( timearray[0], np.float64,
                            exception_message=htype.generate_exception_message( "Waveform.adjust_integration_limits", 
                                                                                60003))
        if not bool(np.prod(np.argsort(timearray)==np.arange(np.shape(timearray)[0]))): # This condition is met 
                                                                                        # if the array is not sorted
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "Waveform.adjust_integration_limits",
                                                                                    60004))
        
        if ill_candidate is not None:
            htype.check_type(   ill_candidate, float, np.float64,
                                exception_message=htype.generate_exception_message( "Waveform.adjust_integration_limits", 
                                                                                    60005))
            ill_candidate_ = Waveform.force_to_range(ill_candidate, min=timearray[0], max=timearray[-1])
            ill_idx = (np.abs(timearray - ill_candidate_)).argmin()
        else:
            ill_idx = 0

        if iul_candidate is not None:
            htype.check_type(   iul_candidate, float, np.float64,
                                exception_message=htype.generate_exception_message( "Waveform.adjust_integration_limits", 
                                                                                    60006))
            iul_candidate_ = Waveform.force_to_range(iul_candidate, min=timearray[ill_idx], max=timearray[-1])
            iul_idx = (np.abs(timearray - iul_candidate_)).argmin()
        else:
            iul_idx = np.shape(timearray)[0]-1

        integrated_points = (iul_idx-ill_idx)+1 # Number of points which will be 
                                                # potentially used for integration

        if integrated_points==1:            # By definition, integrated_points>=1. 
                                            # If integrated_points==1, then iul_idx==ill_idx
            aux = np.delete(timearray, ill_idx)     # Find the entry which is the 
                                                    # nearest one to timearray[ill_idx]
            aux_idx = (np.abs(aux - timearray[ill_idx])).argmin()
            if aux_idx >= ill_idx:      # The new index, aux_idx, is referred to aux,
                iul_idx = aux_idx+1     # which is timearray without timearray[ill_idx].
                                        # We need to correct aux_idx to take this into
                                        # account, so that it points to the correct entry 
                                        # in the original timearray.
            else:                                       # The second closest entry, pointed to by aux_idx,
                ill_idx, iul_idx = aux_idx, ill_idx     # could have been smaller than timearray[ill_idx]
        return ill_idx, iul_idx

    @staticmethod
    def force_to_range(x, min=0., max=1.):
        ...
        htype.check_type(   x, int, float, np.float64,
                            exception_message=htype.generate_exception_message( "Waveform.force_to_range", 
                                                                                70001))
        htype.check_type(   min, int, float, np.float64,
                            exception_message=htype.generate_exception_message( "Waveform.force_to_range", 
                                                                                70002))
        htype.check_type(   max, int, float, np.float64,
                            exception_message=htype.generate_exception_message( "Waveform.force_to_range", 
                                                                                70003))
        if min>max:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "Waveform.force_to_range", 
                                                                                    70004))
        if x<min:
            return float(min)
        elif x<max:
    ...
    def filter_infs_and_nans(input, get_mask=False):
        ...
        htype.check_type(   input, np.ndarray, 
                            exception_message=htype.generate_exception_message( "Waveform.filter_infs_and_nans", 
                                                                                80001))
        if np.ndim(input)!=1:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "Waveform.filter_infs_and_nans", 
                                                                                    80002))
        if input.dtype!=np.float64:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "Waveform.filter_infs_and_nans", 
                                                                                    80003))
        htype.check_type(   get_mask, bool,
                            exception_message=htype.generate_exception_message( "Waveform.filter_infs_and_nans", 
                                                                                80004))
        
        mask = [not (math.isnan(input[i]) or math.isinf(input[i])) for i in range(len(input))]
