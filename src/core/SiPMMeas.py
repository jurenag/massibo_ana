...
class SiPMMeas(ABC):
    ...
    def fit_piecewise_gaussians_to_the_n_highest_peaks( samples,
        ...
        if len(samples)<(3*len(peaks_to_fit_)): # We need at least 3 samples
                                                # per gaussian fit (3 free 
                                                # parameters per gaussian)

            raise cuex.NoAvailableData(htype.generate_exception_message("SiPMMeas.fit_piecewise_gaussians_to_the_n_highest_peaks",
                                                                        34723,
    ...
    def output_summary(self,    folderpath,
        ...
        if indent is not None:
            ...
            htype.check_type(   indent, int, np.int64, str,
                                exception_message=htype.generate_exception_message("SiPMMeas.output_summary", 87057))
            
            if isinstance(indent, int) or isinstance(indent, np.int64):
                if indent<0:
                    raise cuex.InvalidParameterDefinition(htype.generate_exception_message("SiPMMeas.output_summary", 68241))
