...
class WaveformSet(OneTypeRTL):
    ...
    def __init__(self, *waveforms, set_name=None, ref_datetime=None):
        ...
        if len(waveforms)<1:
            raise cuex.NoAvailableData(htype.generate_exception_message("WaveformSet.__init__", 10001))

        for i in range(len(waveforms)):
            htype.check_type( waveforms[i], Waveform,
                                exception_message=htype.generate_exception_message("WaveformSet.__init__", 10002))
        if set_name is not None:
            htype.check_type( set_name, str,
                                exception_message=htype.generate_exception_message("WaveformSet.__init__", 10003))
            self.__set_name_is_available = True
        if ref_datetime is not None:
            htype.check_type( ref_datetime, dt.datetime,
                                exception_message=htype.generate_exception_message("WaveformSet.__init__", 10004))
        ...
        self.__set_time_window = self[-1].T0+self[-1].Time[-1]  # The set window time-width matches
    ...
    def RefDatetime(self):
        if self.__ref_datetime_is_available:
            ...
            raise cuex.NoAvailableData(htype.generate_exception_message("WaveformSet.RefDatetime", 20001))
        
    @property
    def SetTimeWindow(self):
        return self.__set_time_window
    
    def recompute_first_peak_baseline_of_the_whole_wvfset(self, signal_fraction_for_median_cutoff = 0.2):
        ...
        htype.check_type(   signal_fraction_for_median_cutoff, float, np.float64,
                            exception_message=htype.generate_exception_message( "WaveformSet.recompute_first_peak_baseline_of_the_whole_wvfset",
                                                                                30001))
        if signal_fraction_for_median_cutoff<0. or signal_fraction_for_median_cutoff>1.:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.recompute_first_peak_baseline_of_the_whole_wvfset",
                                                                                    30002))
        for wvf in self:
            wvf.compute_first_peak_baseline(signal_fraction_for_median_cutoff=signal_fraction_for_median_cutoff)
        return
    ...
    def mean_waveform(self, amplitude_range=None, integral_range=None):
        ...
        if len(self)==0:
            raise cuex.NoAvailableData(htype.generate_exception_message(   "WaveformSet.mean_waveform",
                                                                            40001,
        ...
        if amplitude_range is not None:
            htype.check_type(   amplitude_range, list,
                                exception_message=htype.generate_exception_message(   "WaveformSet.mean_waveform",
                                                                                        40002))
            if len(amplitude_range)!=2:
                raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.mean_waveform",
                                                                                        40003))
            for i in range(len(amplitude_range)):
                htype.check_type(   amplitude_range[i], float, np.float64,
                                    exception_message=htype.generate_exception_message(   "WaveformSet.mean_waveform",
                                                                                            40004))
        ...
        if not fAmplitudeIsDefined:
            if integral_range is not None:
                htype.check_type(   integral_range, list,
                                    exception_message=htype.generate_exception_message(   "WaveformSet.mean_waveform",
                                                                                            40006))
                ...
                for i in range(len(integral_range)):
                    htype.check_type(   integral_range[i], float, np.float64,
                                        exception_message=htype.generate_exception_message(   "WaveformSet.mean_waveform",
                                                                                                40008))
        ...
        if fAmplitudeIsDefined:
            for i in range(len(self)):
                ith_amplitude = np.max(self[i].Signal)-self[i].Signs['first_peak_baseline']
                if ith_amplitude>=amplitude_range[0] and ith_amplitude<=amplitude_range[1]:
                    filtered_wvfs_idx.append(i)
        else:
            for i in range(len(self)):
                ith_integral = self[i].Integral # If self[i] had not been integrated previously,
                                                # then self[i].Integral will be None
                try:
                    if ith_integral>=integral_range[0] and ith_integral<=integral_range[1]:
                        filtered_wvfs_idx.append(i)

                except TypeError:       # This is what happens if you try to evaluate if None
        ...
        sum = copy.copy(self[filtered_wvfs_idx[0]].Signal)  # We have made sure that there's at least one waveform in this waveform set
        for i in range(1, len(filtered_wvfs_idx)):        
            sum += self[filtered_wvfs_idx[i]].Signal
        return sum/len(filtered_wvfs_idx)

    def plot(self,  wvfs_to_plot=None, 
                    plot_peaks=True,
                    randomize=False,
                    fig_title=None,
                    mode='grid',
        ...
        htype.check_type(   plot_peaks, bool,
                            exception_message=htype.generate_exception_message( "WaveformSet.plot",
                                                                                50001))
        htype.check_type(   randomize, bool,
                            exception_message=htype.generate_exception_message( "WaveformSet.plot",
                                                                                50002))
        if wvfs_to_plot is None:
            ...
            if wvfs_to_plot<1 or wvfs_to_plot>self.__len__():
                raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.plot",
                                                                                        50003))
            if randomize:
                wvfs_indices = tuple(WaveformSet.get_random_indices(self.__len__(), wvfs_to_plot))
            else:
                wvfs_indices = tuple(range(0, wvfs_to_plot))
        elif type(wvfs_to_plot)==list:
            for i in range(len(wvfs_to_plot)):
                htype.check_type( wvfs_to_plot[i], int, np.int64,
                                    exception_message=htype.generate_exception_message(   "WaveformSet.plot",
                                                                                            50004))
        ...
        if fig_title is not None:
            htype.check_type(   fig_title, str,
                                exception_message=htype.generate_exception_message( "WaveformSet.plot",
                                                                                    50007))
        htype.check_type(   mode, str,
                            exception_message=htype.generate_exception_message( "WaveformSet.plot",
                                                                            50008))
        htype.check_type(   nrows, int, np.int64,
                            exception_message=htype.generate_exception_message( "WaveformSet.plot",
                                                                                50009))
        if nrows<2:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.plot",
                                                                                    50010))
        htype.check_type(   ncols, int, np.int64,
                            exception_message=htype.generate_exception_message( "WaveformSet.plot",
                                                                                50011))
        if ncols<2:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.plot",
        ...
        if  ylim is not None:
            ...
            if ylim[0]>=ylim[1]:
                raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.plot",
                                                                                        50020))
        htype.check_type(   wvf_linewidth, float, np.float64,
                            exception_message=htype.generate_exception_message( "WaveformSet.plot",
                                                                                50021))
        if wvf_linewidth<=0.0:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.plot",
                                                                                    50022))
        htype.check_type(   x0, list,
                            exception_message=htype.generate_exception_message( "WaveformSet.plot", 
                                                                                50023))
        for elem in x0:
            htype.check_type(   elem, float, np.float64,
                                exception_message=htype.generate_exception_message( "WaveformSet.plot", 
                                                                                    50024))
        htype.check_type(   y0, list,
                            exception_message=htype.generate_exception_message( "WaveformSet.plot", 
                                                                                50025))
        for elem in y0:
            htype.check_type(   elem, float, np.float64,
        ...
        if mode!='superposition':   # Grid plot is default
            ...
            for i in range(how_many_canvases):
                ...
                for j in range(nrows*ncols):
                    ...
                    try:
                        self.Members[wvfs_indices[counter]].plot(   axs[iterator],
                                                                    xlim=xlim,
                                                                    ylim=ylim,
                        ...
                        WaveformSet.set_custom_labels_visibility(axs[iterator], iterator[0], iterator[1], nrows)
                    except IndexError:  # If the number of waveforms to plot is not a multiple
    ...
    def set_custom_labels_visibility(ax, ax_i, ax_j, nrows):
        ...
        *Note that this method is similar to matplotlib.axes.Axes.label_outer.
        ...
        htype.check_type( ax_i, int,
                            exception_message=htype.generate_exception_message(   "WaveformSet.set_custom_labels_visibility",
                                                                                    60001))
        htype.check_type( ax_j, int,
                            exception_message=htype.generate_exception_message(   "WaveformSet.set_custom_labels_visibility",
                                                                                    60002))
        htype.check_type( nrows, int,
                            exception_message=htype.generate_exception_message(   "WaveformSet.set_custom_labels_visibility",
                                                                                    60003))
        if nrows<1:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.set_custom_labels_visibility",
                                                                                    60004))
    ...
    def get_random_indices(i_max, how_many_samples):
        ...
        htype.check_type( i_max, int,
                            exception_message=htype.generate_exception_message(   "WaveformSet.get_random_indices",
                                                                                    70001))
        if i_max<1:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.get_random_indices",
                                                                                    70002))
        htype.check_type(how_many_samples, int,
                           exception_message=htype.generate_exception_message(    "WaveformSet.get_random_indices",
                                                                                    70003))
    ...
    def index_generator(i_max, j_max):
        ...
        htype.check_type( i_max, int,
                            exception_message=htype.generate_exception_message(   "WaveformSet.index_generator",
                                                                                    80001))
        if i_max<1:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.index_generator",
                                                                                    80002))
        htype.check_type( i_max, int,
                            exception_message=htype.generate_exception_message(   "WaveformSet.index_generator",
                                                                                    80003))
    ...
    @classmethod
    def from_files(cls, input_filepath, time_resolution,
                   points_per_wvf=None, wvfs_to_read=None, separator=None, 
                   timestamp_filepath=None, delta_t_wf=None,
                   set_name=None,
                   ref_datetime=None, creation_dt_offset_min=None,
                   wvf_extra_info=None):
        ...
        htype.check_type( input_filepath, str,
                            exception_message=htype.generate_exception_message( "WaveformSet.from_files",
                                                                                90001))
        if set_name is not None:
            htype.check_type( set_name, str,
                                exception_message=htype.generate_exception_message( "WaveformSet.from_files",
        ...
        if ref_datetime is not None:
            ...
            ref_datetime_ = os.path.getctime(input_filepath)    # Seconds passed between the epoch
        ...
        if creation_dt_offset_min is not None:
            htype.check_type( creation_dt_offset_min, float, np.float64,
                                exception_message=htype.generate_exception_message( "WaveformSet.from_files",
                                                                                    90004))
            ...
            ref_datetime_ = ref_datetime_ + dt.timedelta(  minutes=minutes_offset, 
                                                        seconds=seconds_offset)
            
        if wvf_extra_info is not None:
            htype.check_type(   wvf_extra_info, str,
                                exception_message=htype.generate_exception_message( "WaveformSet.from_files",
                                                                                    90005))
        ...
        waveforms_dict = WaveformSet.read_wvfs( input_filepath,
                                                points_per_wvf=points_per_wvf, wvfs_to_read=wvfs_to_read, separator=separator,
                                                timestamp_filepath=timestamp_filepath, delta_t_wf=delta_t_wf)
        
        waveforms_pack = []
        for key in waveforms_dict.keys():
            signal_holder = np.array(waveforms_dict[key])
            waveform_holder = Waveform( key,
                                        signal_holder, 
    ...
    def read_wvfs(  input_filepath, 
        ...
        htype.check_type( input_filepath, str,
                            exception_message=htype.generate_exception_message(   "WaveformSet.read_wvfs", 
                                                                                    100001))
        fUseNumPoints = False
        if points_per_wvf is not None:
            htype.check_type( points_per_wvf, int,
                                exception_message=htype.generate_exception_message(   "WaveformSet.read_wvfs", 
                                                                                        100002))
            if points_per_wvf<1:
                raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.read_wvfs", 
                                                                                        100003))
            fUseNumPoints = True

        if not fUseNumPoints:   #In this case, separator must be available
            if separator is None:
                raise cuex.InsufficientParameters(htype.generate_exception_message( "WaveformSet.read_wvfs", 
                                                                                        100004))
        ...
        if timestamp_filepath is not None:
            htype.check_type( timestamp_filepath, str,
                                exception_message=htype.generate_exception_message(   "WaveformSet.read_wvfs",
                                                                                        100007))
            fUseTStamp = True
        ...
        if not fUseTStamp:      #In this case, delta_t_wf must be available
            if delta_t_wf is None:
                raise cuex.InsufficientParameters(htype.generate_exception_message( "WaveformSet.read_wvfs",
                                                                                    100008))
            htype.check_type( delta_t_wf, float, np.float64,
                                exception_message=htype.generate_exception_message(   "WaveformSet.read_wvfs",
                                                                                        100009))
            if delta_t_wf<=0.:
                raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.read_wvfs",
        ...
        if fUseNumPoints and fUseTStamp:
            return WaveformSet.read_wvfs_with_ppw_with_timestamp(   input_filepath, 
                                                                    points_per_wvf, 
                                                                    timestamp_filepath)
        elif fUseNumPoints and not fUseTStamp:
            return WaveformSet.read_wvfs_with_ppw_without_timestamp(    input_filepath, 
                                                                        points_per_wvf, 
                                                                        delta_t_wf, 
                                                                        wvfs_to_read=wvfs_to_read)
        elif not fUseNumPoints and fUseTStamp:
            return WaveformSet.read_wvfs_with_separator_with_timestamp( input_filepath, 
                                                                        separator, 
                                                                        timestamp_filepath)
        else:
            return WaveformSet.read_wvfs_with_separator_without_timestamp(  input_filepath, 
                                                                            separator, 
                                                                            delta_t_wf)

    @staticmethod
    def read_wvfs_with_ppw_with_timestamp(input_filepath, points_per_wvf, timestamp_filepath):

        """This static method is meant to be called from read_wvfs(). Refer to its docstring for
        more information."""

        htype.check_type( input_filepath, str,
                            exception_message=htype.generate_exception_message(   "WaveformSet.read_wvfs_with_ppw_with_timestamp", 
                                                                                    110001))
        ...
        htype.check_type( points_per_wvf, int,
                            exception_message=htype.generate_exception_message(   "WaveformSet.read_wvfs_with_ppw_with_timestamp", 
                                                                                    110002))
        if points_per_wvf<1:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.read_wvfs_with_ppw_with_timestamp", 
                                                                                    110003))
        ...
        with open(timestamp_filepath, 'r') as ts_file:
            with open(input_filepath, 'r') as wf_file:
                ...
                key = 0.    #Throw away first entry in the time stamp
                WaveformSet.read_next_wvf_to_dict(running_dict, wf_file, key, points_per_wvf=points_per_wvf)  
                
                for ts_line in ts_file:
                    key = key+float(ts_line.strip())    # Assuming every entry in the time stamp
    ...
    @staticmethod
    def read_wvfs_with_ppw_without_timestamp(input_filepath, points_per_wvf, delta_t_wf, wvfs_to_read=None):

        """This static method is meant to be called from read_wvfs(). Refer to its docstring for
        more information."""

        htype.check_type( input_filepath, str,
                            exception_message=htype.generate_exception_message(   "WaveformSet.read_wvfs_with_ppw_without_timestamp", 
                                                                                    120001))
        htype.check_type( points_per_wvf, int,
                            exception_message=htype.generate_exception_message(   "WaveformSet.read_wvfs_with_ppw_without_timestamp", 
                                                                                    120002))
        if points_per_wvf<1:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.read_wvfs_with_ppw_without_timestamp", 
                                                                                    120003))
        htype.check_type( delta_t_wf, float, np.float64,
                            exception_message=htype.generate_exception_message(   "WaveformSet.read_wvfs_with_ppw_without_timestamp", 
                                                                                    120004))
        ...
        if wvfs_to_read is not None:
            htype.check_type( wvfs_to_read, int,
                                exception_message=htype.generate_exception_message(   "WaveformSet.read_wvfs_with_ppw_without_timestamp", 
                                                                                        120006))
            if wvfs_to_read<0:
                raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.read_wvfs_with_ppw_without_timestamp", 
        ...
        with open(input_filepath, 'r') as file:
            ...
            for i in range(wvfs_to_read_):      
                WaveformSet.read_next_wvf_to_dict(running_dict, file, running_t, points_per_wvf=points_per_wvf)
                running_t += delta_t_wf
        return running_dict

    @staticmethod
    def read_wvfs_with_separator_with_timestamp(input_filepath, separator, timestamp_filepath):

        """This static method is meant to be called from read_wvfs(). Refer to its docstring for
        more information."""

        htype.check_type( input_filepath, str,
                            exception_message=htype.generate_exception_message(   "WaveformSet.read_wvfs_with_separator_with_timestamp",
                                                                                    130001))
        htype.check_type( separator, str,
                            exception_message=htype.generate_exception_message(   "WaveformSet.read_wvfs_with_separator_with_timestamp",
                                                                                    130002))
        if len(separator)<1:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.read_wvfs_with_separator_with_timestamp",
                                                                                    130003))
        htype.check_type( timestamp_filepath, str,
                            exception_message=htype.generate_exception_message(   "WaveformSet.read_wvfs_with_separator_with_timestamp",
                                                                                    130004))
        ...
        with open(timestamp_filepath, 'r') as ts_file:
            with open(input_filepath, 'r') as wf_file:
                key = ts_file.readline()  
                key = 0.    #Throw away first entry in the time stamp
                WaveformSet.read_next_wvf_to_dict(running_dict, wf_file, key, separator=separator)  
                for ts_line in ts_file:
                    key = key+float(ts_line.strip())    # Assuming every entry in the time stamp
                                                        # is measured with respect to the ocurrence
                                                        # of the immediately previous waveform.
    ...
    @staticmethod
    def read_wvfs_with_separator_without_timestamp(input_filepath, separator, delta_t_wf):

        """This static method is meant to be called from read_wvfs(). Refer to its docstring for
        more information."""

        htype.check_type( input_filepath, str,
                            exception_message=htype.generate_exception_message(   "WaveformSet.read_wvfs_with_separator_without_timestamp",
                                                                                    140001))
        htype.check_type( separator, str,
                            exception_message=htype.generate_exception_message(   "WaveformSet.read_wvfs_with_separator_without_timestamp",
                                                                                    140002))
        if len(separator)<1:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.read_wvfs_with_separator_without_timestamp",
                                                                                    140003))
        htype.check_type( delta_t_wf, float, np.float64,
                            exception_message=htype.generate_exception_message(   "WaveformSet.read_wvfs_with_separator_without_timestamp", 
                                                                                    140004))
        ...
        with open(input_filepath, 'r') as wf_file:
            key = 0.
            while WaveformSet.read_next_wvf_to_dict(running_dict, wf_file, key, separator=separator):
                key += delta_t_wf
        return running_dict   
    ...
    def line_counter(filepath):
        ...
        htype.check_type( filepath, str,
                            exception_message=htype.generate_exception_message("WaveformSet.line_counter", 51381))
    ...
    @staticmethod
    def read_next_wvf_to_dict(running_dict, open_file, new_key, points_per_wvf=None, separator=None):
        ...
        htype.check_type( running_dict, dict,
                            exception_message=htype.generate_exception_message(   "WaveformSet.read_next_wvf_to_dict",
                                                                                    150001))
        htype.check_type( new_key, float, np.float64,
                            exception_message=htype.generate_exception_message(   "WaveformSet.read_next_wvf_to_dict",
                                                                                    150002))
        ...
        if points_per_wvf is not None:
            htype.check_type( points_per_wvf, int,
                                exception_message=htype.generate_exception_message(   "WaveformSet.read_next_wvf_to_dict",
                                                                                        150003))
            if points_per_wvf<1:
                raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.read_next_wvf_to_dict", 
                                                                                        150004))
            fUseNumPoints = True
        
        if not fUseNumPoints:
            if separator is None:
                raise cuex.InsufficientParameters(htype.generate_exception_message( "WaveformSet.read_next_wvf_to_dict",
                                                                                    150005))
            htype.check_type( separator, str,
                                exception_message=htype.generate_exception_message(   "WaveformSet.read_next_wvf_to_dict",
                                                                                        150006))
        ...
        if fUseNumPoints:
            for i in range(points_per_wvf):
                ...
                except ValueError:              # This is what happens when you read the
                    fThereAreMoreLines = False  # last line of the file, which is "''",
                                                # and try to convert it into a float.
                    if len(new_value)==0:           # If we have reached the end of
                        return fThereAreMoreLines   # the file and not even one entry
                                                    # has been added to new_value,              
                    else:                           # there's no reason to add an empty
                        break                       # waveform to the dictionary.
                                            
        else:
            try:                                                        # Try getting the first entry, 
                new_candidate = float((open_file.readline()).strip())   # but there may be not even one
            except ValueError:                                          # entry. Also, I am assuming that
                return False                                            # the first entry won't be a separator.                                                          
            while True:
    ...
    def bubble_sort(list_of_scalars, sort_increasingly=True):
        ...
        htype.check_type(   list_of_scalars, list,
                            exception_message=htype.generate_exception_message( "WaveformSet.bubble_sort", 
                                                                                160001))
        for elem in list_of_scalars:
            htype.check_type(   elem, int, np.int64, float, np.float64,
                                exception_message=htype.generate_exception_message( "WaveformSet.bubble_sort", 
                                                                                    160002))
        htype.check_type(   sort_increasingly, bool,
                            exception_message=htype.generate_exception_message( "WaveformSet.bubble_sort", 
                                                                                160003))
        ...
        for i in reversed(range(1, len(samples_))):
            for j in range(0, i):
                if samples_[j+1]<samples_[j]:
                    samples_[j], samples_[j+1] = WaveformSet.swap(samples_[j], samples_[j+1])
    ...
    def purge(self, wvfs_to_erase=[], ask_for_confirmation=True):
        ...
        htype.check_type(   wvfs_to_erase, list,
                            exception_message=htype.generate_exception_message( "WaveformSet.purge", 
                                                                                170001))
        for elem in wvfs_to_erase:
            ...
            if elem<0:
                raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.plot",
                                                                                        170003))
        htype.check_type(   ask_for_confirmation, bool,
                            exception_message=htype.generate_exception_message( "WaveformSet.purge", 
                                                                                170004))
        wvfs_to_erase_ = list(set(wvfs_to_erase))       # Remove redundancy
        wvfs_to_erase_ = WaveformSet.bubble_sort(   wvfs_to_erase_, 
                                                    sort_increasingly=True) # Sort increasingly
        fErase = True
        if ask_for_confirmation:
            fErase = DataPreprocessor.yes_no_translator(input(f"In WaveformSet.purge(): Do you want to proceed to the deletion of the waveforms with the following indices? \n {wvfs_to_erase_}"))

        if fErase:
            wvfs_to_erase_.reverse()    # Sort decreasingly
            for idx in wvfs_to_erase_:  # wvfs_to_erase_ must be sorted decreasingly
                try:
                    del self[idx]
                except IndexError:  # Happens if the user gave some iterator 
                    continue        # value which exceeds len(self). Ignore it.
        return
    ...
    def filter(self,    filter_function,
        ...
        if not callable(filter_function):
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.filter",
                                                                                    180001))
        ...
        htype.check_type(   return_idcs, bool,
                            exception_message=htype.generate_exception_message( "WaveformSet.filter", 
                                                                                180006))
        htype.check_type(   purge, bool,
                            exception_message=htype.generate_exception_message( "WaveformSet.filter", 
                                                                                180007))
        htype.check_type(   ask_for_confirmation, bool,
                            exception_message=htype.generate_exception_message( "WaveformSet.filter", 
        ...
        for i in range(len(self)):
            try:
                ...
                htype.check_type(   fPassed, bool, np.bool_,
                                    exception_message=htype.generate_exception_message( "WaveformSet.filter", 
                                                                                        180009,
            ...
            except TypeError:   # Happens if the given keyword 
        ...
        if purge:
            self.purge( wvfs_to_erase=wvfs_to_erase, 
    ...
    @staticmethod
    def threshold_filter(   waveform: Waveform, 
                            threshold,
        ...
        htype.check_type(   waveform, Waveform,
                            exception_message=htype.generate_exception_message( "WaveformSet.threshold_filter", 
                                                                                190001))
        htype.check_type(   threshold, float, np.float64,
                            exception_message=htype.generate_exception_message( "WaveformSet.threshold_filter", 
                                                                                190002))
        htype.check_type(   threshold_from_baseline, bool,
                            exception_message=htype.generate_exception_message( "WaveformSet.threshold_filter", 
                                                                                190003))
        htype.check_type(   rise, bool,
                            exception_message=htype.generate_exception_message( "WaveformSet.threshold_filter", 
                                                                                190004))
        htype.check_type(   filter_out, bool,
                            exception_message=htype.generate_exception_message( "WaveformSet.threshold_filter", 
                                                                                190005))
        fUseISubrange = False
        if i_subrange is not None:
            htype.check_type(   i_subrange, tuple,
                                exception_message=htype.generate_exception_message( "WaveformSet.threshold_filter", 
                                                                                    190006))
            if len(i_subrange)!=2:
                raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.filter",
                                                                                        190007))
            htype.check_type(   i_subrange[0], int, np.int64,
                                exception_message=htype.generate_exception_message( "WaveformSet.threshold_filter", 
                                                                                    190008))
            htype.check_type(   i_subrange[1], int, np.int64,
                                exception_message=htype.generate_exception_message( "WaveformSet.threshold_filter", 
                                                                                    190009))

            if i_subrange[0]<0 or i_subrange[0]>len(waveform.Time)-1 or i_subrange[1]<0 or i_subrange[1]>len(waveform.Time)-1:
                raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.filter",
                                                                                        190010))
        ...
        fUseSubrange = False
        if not fUseISubrange:   # Only pay attention to subrange if i_subrange is not defined
            if subrange is not None:
                htype.check_type(   subrange, tuple,
                                    exception_message=htype.generate_exception_message( "WaveformSet.threshold_filter", 
                                                                                        190012))
                if len(subrange)!=2:
                    raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.filter",
                                                                                            190013))
                htype.check_type(   subrange[0], float, np.float64,
                                    exception_message=htype.generate_exception_message( "WaveformSet.threshold_filter", 
                                                                                        190014))
                htype.check_type(   subrange[1], float, np.float64,
                                    exception_message=htype.generate_exception_message( "WaveformSet.threshold_filter", 
                                                                                        190015))

                aux_min, aux_max = np.min(waveform.Time), np.max(waveform.Time)

                if subrange[0]<aux_min or subrange[0]>aux_max or subrange[1]<aux_min or subrange[1]>aux_max:
                    raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.filter",
                                                                                            190016))
                if subrange[0]>=subrange[1]:
                    raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.filter",
                                                                                            190017))
                fUseSubrange = True # Use subrange only if not fUseISubrange 
        ...
        else:   # Note that fUseISubrange and fUseSubrange 
        ...
        if rise:
            result = (np.max(waveform.Signal[i_low:i_up+1])>=threshold_) # Make the upper limit inclusive
        else:
            result = (np.min(waveform.Signal[i_low:i_up+1])<=threshold_) # Make the upper limit inclusive
    ...
    @staticmethod
    def correlation_filter( waveform: Waveform, 
                            threshold, 
                            model_y, 
                            i0=0, 
                            delta_t=None,
                            also_return_correlation=False) -> bool:
        ...
        htype.check_type(   waveform, Waveform,
                            exception_message=htype.generate_exception_message( "WaveformSet.correlation_filter", 
                                                                                200001))
        htype.check_type(   threshold, float, np.float64,
                            exception_message=htype.generate_exception_message( "WaveformSet.correlation_filter", 
                                                                                200002))
        htype.check_type(   model_y, np.ndarray,
                            exception_message=htype.generate_exception_message( "WaveformSet.correlation_filter", 
                                                                                200003))
        if model_y.dtype!=np.float64:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.correlation_filter",
                                                                                    200004))
        if model_y.ndim!=1:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.correlation_filter",
                                                                                    200005))
        if len(model_y)!=len(waveform.Time):
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.correlation_filter",
                                                                                    200006))
        htype.check_type(   i0, int, np.int64,
                            exception_message=htype.generate_exception_message( "WaveformSet.correlation_filter", 
                                                                                200007))
        if i0<0:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.correlation_filter",
        ...
        if delta_t is not None:
            htype.check_type(   delta_t, float, np.float64,
                                exception_message=htype.generate_exception_message( "WaveformSet.correlation_filter", 
                                                                                    200009))
            if delta_t<=0.0:
                raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.correlation_filter",
        ...
        htype.check_type(   also_return_correlation, bool,
                            exception_message=htype.generate_exception_message( "WaveformSet.correlation_filter", 
                                                                                200011))
        if fUseDeltaT:
            iN, _ = search.find_nearest_neighbour(waveform.Time, waveform.Time[i0]+delta_t)
            if iN<=i0:
                raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.correlation_filter",
                                                                                        200012,
    ...
    @staticmethod
    def integral_filter(waveform: Waveform, 
                        threshold, 
                        i0=0, 
                        delta_t=None,
                        also_return_integral=False) -> bool:
        ...
        htype.check_type(   waveform, Waveform,
                            exception_message=htype.generate_exception_message( "WaveformSet.integral_filter", 
                                                                                210001))
        htype.check_type(   threshold, float, np.float64,
                            exception_message=htype.generate_exception_message( "WaveformSet.integral_filter", 
                                                                                210002))
        htype.check_type(   i0, int, np.int64,
                            exception_message=htype.generate_exception_message( "WaveformSet.integral_filter", 
                                                                                210003))
        if i0<0:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.integral_filter",
                                                                                    210004))
        fUseDeltaT = False
        if delta_t is not None:
            htype.check_type(   delta_t, float, np.float64,
        ...
        htype.check_type(   also_return_integral, bool,
                            exception_message=htype.generate_exception_message( "WaveformSet.integral_filter", 
                                                                                210007))
        if not fUseDeltaT:
            result = waveform.integrate(integration_lower_lim=waveform.Time[i0], 
    ...
    @staticmethod
    def peaks_no_filter(waveform: Waveform,
                        n_peaks,
                        filter_out_below=True) -> bool:
        ...
        htype.check_type(   waveform, Waveform,
                            exception_message=htype.generate_exception_message( "WaveformSet.peaks_no_filter", 
                                                                                220001))
        htype.check_type(   n_peaks, int, np.int64,
                            exception_message=htype.generate_exception_message( "WaveformSet.peaks_no_filter", 
                                                                                220002))
        htype.check_type(   filter_out_below, bool,
                            exception_message=htype.generate_exception_message( "WaveformSet.peaks_no_filter", 
                                                                                220003))
        if filter_out_below:
    ...
    def exclude_these_indices(self, idcs_to_exclude):
        ...
        htype.check_type(   idcs_to_exclude, list,
                            exception_message=htype.generate_exception_message( "WaveformSet.exclude_these_indices", 
                                                                                230001))
        for elem in idcs_to_exclude:
            ...
            if elem<0 or elem>=len(self):
                raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.exclude_these_indices",
                                                                                        230003))
        idcs_to_exclude_ = list(set(idcs_to_exclude))                       # Remove redundancy
        idcs_to_exclude_ = WaveformSet.bubble_sort( idcs_to_exclude_,
                                                    sort_increasingly=True) # Sort increasingly
        result = []
        pointer = 0
        for i in range(0, len(self)):           
            if pointer<len(idcs_to_exclude_):       # I can use this method to populate result
                if i!=idcs_to_exclude_[pointer]:    # because I have removed the redundancy and
                    result.append(i)                # I have ordered idcs_to_exclude_ increasingly
                else:
                    pointer += 1
            else:                                       # At this point, we have 
                result += list(range(i, len(self)))     # exhausted idcs_to_exclude,
                break                                   # so add the remaining indices.
            
        return result
    
    def get_average_baseline(self,  get_std=False, 
                                    get_dispersion=False):
        ...
        htype.check_type(   get_std, bool,
                            exception_message=htype.generate_exception_message( "WaveformSet.get_average_baseline", 
                                                                                240001))
        htype.check_type(   get_dispersion, bool,
                            exception_message=htype.generate_exception_message( "WaveformSet.get_average_baseline", 
                                                                                240002))
        ...
        if get_std==False and get_dispersion==False:
            ...
            if len(baselines)<2:
                raise cuex.NoAvailableData(htype.generate_exception_message("WaveformSet.get_average_baseline", 
                                                                            240003,
    ...
    def find_peaks(self, return_peak_properties=False, **kwargs):
        ...
        htype.check_type(   return_peak_properties, bool,
                            exception_message=htype.generate_exception_message( "WaveformSet.find_peaks", 
                                                                                250001))
        if not return_peak_properties:
            for wvf in self:
                ...
                peaks_idx, _ = spsi.find_peaks(wvf.Signal, **kwargs)   # Peak finding algorithm
            
                for idx in peaks_idx:
                    wvf.Signs = ('peaks_pos', [wvf.Time[idx]],      False)   # Add peak info. to waveforms
                    wvf.Signs = ('peaks_top', [wvf.Signal[idx]],    False)

            return
        
        else:           # Duplicating the code so that the return_peak_properties 
            ...
            for wvf in self:
                ...
                peaks_idx, properties = spsi.find_peaks(wvf.Signal, **kwargs)   # Peak finding algorithm
    ...
    def integrate(self, input_resistance_in_ohms=50.,
        ...
        htype.check_type(   input_resistance_in_ohms, float, np.float64,
                            exception_message=htype.generate_exception_message( "WaveformSet.integrate", 
                                                                                260001))
        htype.check_type(   system_amplification_factor, float, np.float64,
                            exception_message=htype.generate_exception_message( "WaveformSet.integrate", 
                                                                                260002))
        if system_amplification_factor<=0.0:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "WaveformSet.integrate", 
                                                                                    260003))
        ...
        result = []
        for wvf in self:
            result.append(wvf.integrate(integration_lower_lim=integration_lower_lim,
                                        integration_upper_lim=integration_upper_lim)/(input_resistance_in_ohms*system_amplification_factor))
        if filter_out_infs_and_nans:
            return Waveform.filter_infs_and_nans(   np.array(result),
