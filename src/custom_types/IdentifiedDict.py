...
class IdentifiedDict(dict):
    ...
    def __init__(   self,   input_data,
        ...
        htype.check_type(   input_data, dict,
                            exception_message=htype.generate_exception_message( "IdentifiedDict.__init__",
                                                                                91127))
    ...
    def assign_identifier(self, id_callable, *args, **kwargs):
        ...
        if not callable(id_callable):
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "IdentifiedDict.assign_identifier",
                                                                                    18253))        
        ...
        if hasattr(self, id_callable.__name__):
            ...
            self.__id = id_callable_(*args, **kwargs)           # When the passed callable is a method 
    ...
    def lowercase_and_remove_redundancy(input_seq):
        ...
        htype.check_type(   input_seq, tuple, list,
                            exception_message=htype.generate_exception_message( "IdentifiedDict.lowercase_and_remove_redundancy", 
                                                                                25103))
        for x in input_seq:
        ...
        for i in reversed(range(len(aux))): # We need to iterate backwards, 
                                            # since, at each iteration, we 
                                            # may delete an entry from aux
            
            if aux[i] in aux[:i]:           # Check whether this element is 
                                            # duplicated in the sub-sequence
                                            # before the current element
                
                del aux[i]                  # If so, this element is duplicated,
                                            # so delete it
    ...
    def from_json_file( cls,    json_filepath,
        ...
        htype.check_type(   json_filepath, str,
                            exception_message=htype.generate_exception_message( "IdentifiedDict.from_json_file", 
                                                                                33808))
        htype.check_type(   ignored_fields, list,
                            exception_message=htype.generate_exception_message( "IdentifiedDict.from_json_file", 
                                                                                22667))
        ...
        if not os.path.exists(json_filepath) or not os.path.isfile(json_filepath):
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "IdentifiedDict.from_json_file", 
                                                                                    67892,
