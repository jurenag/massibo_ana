...
#Constructor
    def __init__(self, list_name=None, member_types=None, init_members=None, restrictive=False):
        ...
        htype.check_type(   restrictive, bool,
                            exception_message=htype.generate_exception_message("TypedList.__init__", 10001))
        
        if list_name is not None:
            htype.check_type(   list_name, str,
                                exception_message=htype.generate_exception_message("TypedList.__init__", 10002))
            
        if member_types is not None:
            htype.check_type(   member_types, list,
                                exception_message=htype.generate_exception_message("TypedList.__init__", 10003))
                    
            for i in range(len(member_types)):
                htype.check_type(   member_types[i], type,
                                    exception_message=htype.generate_exception_message("TypedList.__init__", 10004))
        ...
        if init_members is not None:
            htype.check_type(   init_members, list,
                                exception_message=htype.generate_exception_message("TypedList.__init__", 10005))
...
#Tools
    ...
    def switch_restrictiveness(self):
        ...
        if not self.__restrictiveness_is_locked:
            ...
            raise Exception(htype.generate_exception_message(    "TypedList.switch_restrictiveness", 20001,
    ...
    def append(self, __candidate_object):
        ...
        if fAdd:
            ...
            print("In function TypedList.try_adding_an_element():ERR0: Could not add an element.")
        return fAdd
    ...
    def extend(self, a_list):
        ...
        htype.check_type(   a_list, list, TypedList,
                            exception_message=htype.generate_exception_message(  "TypedList.extend",
                                                                                30001))
    ...
    def remove_member_by_index(self, i):
        ...
        htype.check_type(   i, int, np.int64,
                            exception_message=htype.generate_exception_message("TypedList.remove_member_by_index()", 40001))
        
        if i<0 or i>(self.__len__()-1):
            raise Exception(htype.generate_exception_message(   "TypedList.remove_member_by_index()", 40002,
