...
class RigidKeyDictionary(dict):
    ...
    def __init__(self, potential_keys, is_typed=False, values_types=None):
        ...
        htype.check_type(   potential_keys, list,
                            exception_message=htype.generate_exception_message("RigidKeyDictionary.__init__", 10001))
        fIsList = False
        if type(is_typed)==bool:
            pass
        elif type(is_typed)==list:
            if len(is_typed)!=len(potential_keys):
                raise cuex.InvalidParameterDefinition(htype.generate_exception_message("RigidKeyDictionary.__init__", 10002))
            for element in is_typed:
                htype.check_type(   element, bool,
                                    exception_message=htype.generate_exception_message("RigidKeyDictionary.__init__", 10003))
            fIsList = True
        else:
            raise cuex.TypeException(htype.generate_exception_message("RigidKeyDictionary.__init__", 10004))
        ...
        if not fIgnoreValuesTypes:  # In this case, we have to check 
                                    # the proper format of values_types
            htype.check_type(   values_types, list,
                                exception_message=htype.generate_exception_message("RigidKeyDictionary.__init__", 10005))
            for element in values_types:
                htype.check_type(   element, type, 
                                    exception_message=htype.generate_exception_message("RigidKeyDictionary.__init__", 10006))
            ...
            if len(values_types)!=aux:  # aux is the number of typed potential keys
                raise cuex.InvalidParameterDefinition(htype.generate_exception_message("RigidKeyDictionary.__init__", 10007))

        self.__potential_keys = potential_keys
        self.__values_types = None

        if fIgnoreValuesTypes:  # Either is_typed==False or
                                # is_typed==[False, False, ..., False]
            self.__values_types = [object for x in self.__potential_keys]
        else:
            if not fIsList: # is_typed must be True
                self.__values_types = values_types  #len(values_types)==len(potential_keys)
    ...
    def update(self, a_dict):
        ...
        htype.check_type(   a_dict, dict,
                            exception_message=htype.generate_exception_message( "RigidKeyDictionary.update()", 20001))
