...
class SiPMMeasSummary(IdentifiedDict):
    ...
    def standard_identifier(self,   add_one_to_sipm_location=False,
                                    strip_id_key_candidates=[],
                                    sipm_location_key_candidates=[]) -> str:
        ...
        htype.check_type(   add_one_to_sipm_location, bool,
                            exception_message=htype.generate_exception_message( "SiPMMeasSummary.standard_identifier", 
                                                                                50221))
        htype.check_type(   strip_id_key_candidates, tuple, list,
                            exception_message=htype.generate_exception_message( "SiPMMeasSummary.standard_identifier", 
                                                                                59288))
        htype.check_type(   sipm_location_key_candidates, tuple, list,
                            exception_message=htype.generate_exception_message( "SiPMMeasSummary.standard_identifier", 
                                                                                64548))
        for x in strip_id_key_candidates+sipm_location_key_candidates:
            htype.check_type(   x, str,
                                exception_message=htype.generate_exception_message( "SiPMMeasSummary.standard_identifier", 
                                                                                    76611))
            
        strip_id_key_candidates_ = strip_id_key_candidates.copy()               # Avoid creating a view of the keyword argument,
                                                                                # otherwise modifications to the list would affect
                                                                                # the default value of the keyword argument in 
                                                                                # subsequent calls to this method
        sipm_location_key_candidates_ = sipm_location_key_candidates.copy()

        strip_id_key_candidates_.extend([   'sipm_strip_id', 'sipmstrip_id', 'sipm_stripid', 'sipmstripid',
                                            'strip_id',  'stripid',   'strip',          # Add some 
                                            'strip_number', 'stripnumber', 'stripnum',  # key candidates
                                            'board_id', 'boardid', 'board',             # default values
                                            'board_number', 'boardnumber', 'boardnum'])
        
        sipm_location_key_candidates_.extend([  'sipm_location', 'sipmlocation', 'sipmloc',
                                                'sipm', 'sipmnumber', 'sipm_number'])
        ...
        strip_id_key_candidates_ = IdentifiedDict.lowercase_and_remove_redundancy(strip_id_key_candidates_)
        sipm_location_key_candidates_ = IdentifiedDict.lowercase_and_remove_redundancy(sipm_location_key_candidates_)

        strip_ID = SiPMMeasSummary.try_retrieving_a_value_from_dict(self,   
                                                                    key_candidates=strip_id_key_candidates_,
                                                                    return_successful_key=False)
        
        sipm_location = SiPMMeasSummary.try_retrieving_a_value_from_dict(   self, 
    ...
    @staticmethod
    def find_and_translate_dcr_key( input_dict : typing.Union[dict, IdentifiedDict],    # Not including SiPMMeasSummary here
                                    dcr_key_candidates=[]):                             # because in that case I would need 
                                                                                        # to use quotes (that's known as a 
                                                                                        # forward declaration, and it is needed
                                                                                        # since I am referencing the class from 
                                                                                        # within the class. For more information,
                                                                                        # check https://peps.python.org/pep-0484/#forward-references ),
                                                                                        # otherwise this will yield a NameError.
                                                                                        # I could use a forward reference, but
                                                                                        # inspecting the signature for the 
                                                                                        # type-hints is not straightforward, 
                                                                                        # because what you get at the inspection
                                                                                        # phase is actually a forward-reference,
                                                                                        # not its associated type.
        ...
        htype.check_type(   input_dict, dict, IdentifiedDict, SiPMMeasSummary,
                            exception_message=htype.generate_exception_message( "SiPMMeasSummary.find_and_translate_dcr_key", 
                                                                                16657))
        htype.check_type(   dcr_key_candidates, tuple, list,
                            exception_message=htype.generate_exception_message( "SiPMMeasSummary.find_and_translate_dcr_key", 
                                                                                85356))
        for x in dcr_key_candidates:
            htype.check_type(   x, str,
                                exception_message=htype.generate_exception_message( "SiPMMeasSummary.find_and_translate_dcr_key", 
                                                                                    31884))
        if 'DCR_mHz_per_mm2' in input_dict.keys():  # If the key is already
            return                                  # there do nothing              

        dcr_key_candidates_ = dcr_key_candidates.copy() # Avoid creating a view of the keyword argument,
                                                        # otherwise modifications to the list would affect
                                                        # the default value of the keyword argument in 
                                                        # subsequent calls to this method
        dcr_key_candidates_.extend(['dcr', 
                                    'dcr (mhz/mm2)', 'dcr_mhz_mm2', 'dcr_mhz_per_mm2',
                                    'dcr (mhz/mm^2)', 'dcr_mhz_mm^2', 'dcr_mhz_per_mm^2',
                                    'dcrate',
                                    'dcrate (mhz/mm2)', 'dcrate_mhz_mm2', 'dcr_mhz_per_mm2',
                                    'dcrate (mhz/mm^2)', 'dcrate_mhz_mm^2', 'dcrate_mhz_per_mm^2',
                                    'dc_rate',
                                    'dc_rate (mhz/mm2)', 'dc_rate_mhz_mm2', 'dc_rate_mhz_per_mm2',
                                    'dc_rate (mhz/mm^2)', 'dc_rate_mhz_mm^2', 'dc_rate_mhz_per_mm^2',
                                    'darkcountrate',
                                    'darkcountrate (mhz/mm2)', 'darkcountrate_mhz_mm2', 'darkcountrate_mhz_per_mm2',
                                    'darkcountrate (mhz/mm^2)', 'darkcountrate_mhz_mm^2', 'darkcountrate_mhz_per_mm^2',
                                    'darkcount_rate'
                                    'darkcount_rate (mhz/mm2)', 'darkcount_rate_mhz_mm2', 'darkcount_rate_mhz_per_mm2',
                                    'darkcount_rate (mhz/mm^2)', 'darkcount_rate_mhz_mm^2', 'darkcount_rate_mhz_per_mm^2',
                                    'dark_count_rate',
                                    'dark_count_rate (mhz/mm2)', 'dark_count_rate_mhz_mm2', 'dark_count_rate_mhz_per_mm2', 
                                    'dark_count_rate (mhz/mm^2)', 'dark_count_rate_mhz_mm^2', 'dark_count_rate_mhz_per_mm^2'])
        ...
        dcr_key_candidates_ = IdentifiedDict.lowercase_and_remove_redundancy(dcr_key_candidates_)

        _, dcr_key = SiPMMeasSummary.try_retrieving_a_value_from_dict(  input_dict, 
                                                                        key_candidates=dcr_key_candidates_,
                                                                        return_successful_key=True)  
        ...
        input_dict['DCR_mHz_per_mm2'] = input_dict[dcr_key]     # Add the DCR under the proper key
        del input_dict[dcr_key]                                 # Delete the old key

        return
    
    @staticmethod
    def try_computing_dcr_in_dict(  input_dict : typing.Union[dict, IdentifiedDict],
                                    sipm_surface_mm2,
        ...
        htype.check_type(   sipm_surface_mm2, float, np.float64,
                            exception_message=htype.generate_exception_message( "SiPMMeasSummary.try_computing_dcr_in_dict", 
                                                                                28546))
        htype.check_type(   input_dict, dict, IdentifiedDict, SiPMMeasSummary,
                            exception_message=htype.generate_exception_message( "SiPMMeasSummary.try_computing_dcr_in_dict", 
                                                                                40583))
        htype.check_type(   acquisition_time_key_candidates, tuple, list,
                            exception_message=htype.generate_exception_message( "SiPMMeasSummary.try_computing_dcr_in_dict", 
                                                                                99501))
        htype.check_type(   dc_number_key_candidates, tuple, list,
                            exception_message=htype.generate_exception_message( "SiPMMeasSummary.try_computing_dcr_in_dict", 
                                                                                67381))
        ...
        acquisition_time_key_candidates_ = acquisition_time_key_candidates.copy()   # Avoid creating a view of the keyword argument,
                                                                                    # otherwise modifications to the list would affect
                                                                                    # the default value of the keyword argument in 
                                                                                    # subsequent calls to this method
        dc_number_key_candidates_ = dc_number_key_candidates.copy()

        acquisition_time_key_candidates_.extend([   'acquisition_time', 'acquisition_time_s', 
                                                    'acquisition_time_sec', 'acquisition_time_secs',
                                                    'acquisition_time_seconds'])
        
        dc_number_key_candidates_.extend([  'counts', 'counts_num', 'counts_number',
                                            'dark_counts', 'dark_counts_num', 'dark_counts_number',
                                            'dc_counts', 'dc_counts_num', 'dc_counts_number',
                                            'dc', 'dc_num', 'dc_number'])
        ...
        acquisition_time_key_candidates_ = IdentifiedDict.lowercase_and_remove_redundancy(acquisition_time_key_candidates_)
        dc_number_key_candidates_ = IdentifiedDict.lowercase_and_remove_redundancy(dc_number_key_candidates_)

        fDataNotAvailable = False
        try:
            acquisition_time = SiPMMeasSummary.try_retrieving_a_value_from_dict(input_dict, 
                                                                                key_candidates=acquisition_time_key_candidates_,
                                                                                return_successful_key=False)
            
            dc_number = SiPMMeasSummary.try_retrieving_a_value_from_dict(   input_dict, 
        ...
        if fDataNotAvailable:
            if 'DCR_mHz_per_mm2' in input_dict.keys():
                ...
                raise cuex.NoAvailableData(htype.generate_exception_message("SiPMMeasSummary.try_computing_dcr_in_dict",
                                                                            49988,
            ...
            input_dict['DCR_mHz_per_mm2'] = 1000.*dc_number/(acquisition_time*sipm_surface_mm2)
            return # Add the key to input_dict and return
        
    @staticmethod
    def try_retrieving_a_value_from_dict(input_dict, key_candidates=[], return_successful_key=False):
        ...
        htype.check_type(   input_dict, dict, IdentifiedDict, SiPMMeasSummary,
                            exception_message=htype.generate_exception_message( "SiPMMeasSummary.try_retrieving_a_value_from_dict", 
                                                                                60230))
        htype.check_type(   key_candidates, tuple, list,
                            exception_message=htype.generate_exception_message( "SiPMMeasSummary.try_retrieving_a_value_from_dict", 
                                                                                35664))
        ...
        if desired_value is None:
            raise cuex.NoAvailableData(htype.generate_exception_message("SiPMMeasSummary.try_retrieving_a_value_from_dict",
                                                                        17561,
