...
class SMSSet(OneTypeRTL):
    ...
    def __init__(   self,   sipmmeas_summaries,
                            *args,
                            list_name='default',
        ...
        htype.check_type(   sipmmeas_summaries, tuple, list,
                            exception_message=htype.generate_exception_message( "SMSSet.__init__", 
                                                                                72114))
        for x in sipmmeas_summaries:
            htype.check_type(   x, SiPMMeasSummary,
                                exception_message=htype.generate_exception_message( "SMSSet.__init__", 
                                                                                    98872))
        # list_name is managed by the parent class initializer
            
        htype.check_type(   identifier_to_assign, str,
                            exception_message=htype.generate_exception_message( "SMSSet.__init__", 
                                                                                96733))
        super().__init__(   SiPMMeasSummary, 
                            list_name=list_name,
                            init_members=list(sipmmeas_summaries))
        ...
        for member in self:
            ...
            if identifier_to_assign.lower()=='standard':
                ...
                raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "SMSSet.__init__", 
                                                                                        77490,
            ...
            member.assign_identifier(   identifier_assigner, 
                                        *args, 
                                        **kwargs)
        return
    ...
    def do_for_every_member(self, the_callable, *args, **kwargs):
        ...
        if not callable(the_callable):
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "SMSSet.do_for_every_member",
                                                                                    91372))
        ...
        if isinstance(fp_annotation, typing._UnionGenericAlias):    # Happens if the first parameter is hinted using
                                                                    # a typing.Union, meaning that its type could be
                                                                    # any of the types within an specified set of types
            if not IdentifiedDict in typing.get_args(fp_annotation):
                raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "SMSSet.do_for_every_member",
                                                                                        12280))
        elif fp_annotation!=IdentifiedDict:     # If the first parameter is hinted using just
                                                # one type, then it must be IdentifiedDict
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "SMSSet.do_for_every_member",
                                                                                    42804,
    ...
    def find_by_identifer(self, identifier, verbose=False):
        ...
        if identifier is None:
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "SMSSet.find_by_identifer",
                                                                                    20970))
        htype.check_type(   verbose, bool,
                            exception_message=htype.generate_exception_message( "SMSSet.find_by_identifer",
                                                                                80385))
        for member in self:
            if member.ID is None:
                if verbose:
                    print('In method SMSSet.find_by_identifier(): An unidentified member was spotted (its ID matches None). This member is skipped.')
                continue
            elif member.ID==identifier:
                return member
            
        if verbose:
            print(f"In method SMSSet.find_by_identifier(): No member with the given identifier ({identifier}) was found.")

        return None # This point is reached if no member 
                    # with the given identifier was found
    ...
    def compare(self,   other, 
        ...
        htype.check_type(   other, SMSSet,
                            exception_message=htype.generate_exception_message( "SMSSet.compare",
                                                                                43804))
        htype.check_type(   fields_to_compare, list,
                            exception_message=htype.generate_exception_message( "SMSSet.compare",
                                                                                46975))
        htype.check_type(   comparing_callables, list,
                            exception_message=htype.generate_exception_message( "SMSSet.compare",
                                                                                55749))
        if len(comparing_callables)!=len(fields_to_compare):
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "SMSSet.compare",
                                                                                    75382))
        ...
        for member in self:
            if member.ID is None:
                if verbose:
                    print('In method SMSSet.compare(): An unidentified member was spotted (its ID matches None). This member is skipped.')
            else:
                current_ID = member.ID
                matching_member = other.find_by_identifer(current_ID, verbose=verbose)
                if matching_member is None:     # If no member in other was found to have
                    ...
                    for i in range(len(fields_to_compare)):
                        if fields_to_compare[i] in member.keys() and fields_to_compare[i] in matching_member.keys():
                            output[current_ID][fields_to_compare[i]] = comparing_callables[i](  member[fields_to_compare[i]],
                                                                                                matching_member[fields_to_compare[i]],
                                                                                                *args, **kwargs)
                        elif verbose:
                            print(f"In method SMSSet.compare(): The field {fields_to_compare[i]} was not simultaneously found in both members to be compared. This field is skipped.")       
        return output

    @classmethod
    def from_excel_file(cls,    excel_filepath,
                                *args,
                                sheet_name=0,
                                header=0,
                                usecols=None,
        ...
        htype.check_type(   excel_filepath, str,
                            exception_message=htype.generate_exception_message( "SMSSet.from_excel_file", 
                                                                                54432))
        htype.check_type(   sheet_name, int, np.int64, str,
                            exception_message=htype.generate_exception_message( "SMSSet.from_excel_file", 
                                                                                43201))
        htype.check_type(   header, int, np.int64,
                            exception_message=htype.generate_exception_message( "SMSSet.from_excel_file", 
                                                                                77598))
        if usecols is not None or not callable(usecols):

            htype.check_type(   usecols, str, list,
                                exception_message=htype.generate_exception_message( "SMSSet.from_excel_file", 
                                                                                    95177))
            if type(usecols)==list:
                for x in usecols:
                    htype.check_type(   x, int, str,
                                        exception_message=htype.generate_exception_message( "SMSSet.from_excel_file", 
                                                                                            13233))
        htype.check_type(   list_name, str,
                            exception_message=htype.generate_exception_message( "SMSSet.from_excel_file", 
                                                                                50729))
        htype.check_type(   identifier_to_assign, str,
                            exception_message=htype.generate_exception_message( "SMSSet.from_excel_file", 
                                                                                58836))
        htype.check_type(   ignored_fields, list,
                            exception_message=htype.generate_exception_message( "SMSSet.from_excel_file", 
                                                                                28328))
        for x in ignored_fields:
            htype.check_type(   x, str,
                                exception_message=htype.generate_exception_message( "SMSSet.from_excel_file", 
                                                                                    63706))
        input_df = pd.read_excel(   excel_filepath, sheet_name=sheet_name, 
                                                    header=header, 
                                                    usecols=usecols)
        
        set = [ SiPMMeasSummary(    input_df.iloc[i].to_dict(),
                                    ignored_fields=ignored_fields) for i in range(len(input_df))]
        return cls( set, 
                    *args, 
                    list_name=list_name,
    ...
    @classmethod
    def from_jsons_folder(cls,  jsons_folderpath,
                                *args,
        ...
        htype.check_type(   jsons_folderpath, str,
                            exception_message=htype.generate_exception_message( "SMSSet.from_jsons_folder", 
                                                                                71939))
        if not os.path.isdir(jsons_folderpath):
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "SMSSet.from_jsons_folder",
                                                                                    47368))
        if not callable(filename_filter):
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "SMSSet.from_jsons_folder",
                                                                                    69212))
        signature = inspect.signature(filename_filter)
        if len(signature.parameters)<1:                 # The file-name filters gets at least one argument
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "SMSSet.from_jsons_folder",
                                                                                    30583,
                                                                                    extra_info="The signature of the given file-name filter must have one and only one argument."))
        if list(signature.parameters.keys())[0]!='filename':    # The first argument gotten by the filter is called 'filename'
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "SMSSet.from_jsons_folder",
                                                                                    87610,
                                                                                    extra_info="The name of the only parameter of the signature of the given file-name filter must be 'filename'."))
        
        if signature.parameters['filename'].default!=inspect.Parameter.empty:   # The file-name filter gets no default 
                                                                                # arguments, i.e. it is a positional argument
            raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "SMSSet.from_jsons_folder",
                                                                                    93994,
                                                                                    extra_info="The first parameter of the signature of the given file-name filter must be a positional argument."))
        
        for parameter in list(signature.parameters.keys())[1:]:         # The rest of the parameters must be keyword arguments
            if signature.parameters[parameter].default==inspect.Parameter.empty:
                raise cuex.InvalidParameterDefinition(htype.generate_exception_message( "SMSSet.from_jsons_folder",
                                                                                        67631,
        ...
        htype.check_type(   filter_kwargs, dict,
                            exception_message=htype.generate_exception_message( "SMSSet.from_jsons_folder", 
                                                                                35587))
            ...
            if filename.endswith('.json'):
                if filename_filter(filename, **filter_kwargs):
                    ...
                    if os.path.isfile(filepath):
                        aux.append(SiPMMeasSummary.from_json_file(filepath, ignored_fields=ignored_fields))
    ...
    def absolute_difference(x, y):
        ...
        htype.check_type(   x, int, np.int64, float, np.float64,
                            exception_message=htype.generate_exception_message( "SMSSet.absolute_difference", 
                                                                                44355))
        htype.check_type(   y, int, np.int64, float, np.float64,
                            exception_message=htype.generate_exception_message( "SMSSet.absolute_difference", 
                                                                                33788))
        return abs(x-y)
    ...
    def difference(x, y):
        ...
        htype.check_type(   x, int, np.int64, float, np.float64,
                            exception_message=htype.generate_exception_message( "SMSSet.difference", 
                                                                                81206))
        htype.check_type(   y, int, np.int64, float, np.float64,
                            exception_message=htype.generate_exception_message( "SMSSet.difference", 
                                                                                99411))
    ...
    def filename_startswith(filename : str, prefix='') -> bool:
        ...
        htype.check_type(   filename, str,
                            exception_message=htype.generate_exception_message( "SMSSet.filename_startswith", 
                                                                                71031))
        htype.check_type(   prefix, str,
                            exception_message=htype.generate_exception_message( "SMSSet.filename_startswith", 
                                                                                60163))
