...
class ListsRKD(RigidKeyDictionary):
    ...
    def __init__(self, potential_keys, is_subtyped=False, values_subtypes=None):
        ...
        if type(is_subtyped) == bool:
            ...
            if len(is_subtyped) != len(potential_keys):
                raise cuex.InvalidParameterDefinition(
                    htype.generate_exception_message("ListsRKD.__init__", 10002)
                )
            for element in is_subtyped:
                htype.check_type(
                    ...
                    bool,
                    exception_message=htype.generate_exception_message(
                        "ListsRKD.__init__", 10003
                    ),
                )
            fIsList = True
        else:
            raise cuex.TypeException(
                htype.generate_exception_message("ListsRKD.__init__", 10004)
            )
        ...
        if not fIgnoreValuesSubTypes:
            htype.check_type(
                values_subtypes,
                list,
                exception_message=htype.generate_exception_message(
                    "ListsRKD.__init__", 10005
                ),
            )
            ...
            if len(values_subtypes) != aux:  # aux is the number of typed potential keys
                raise cuex.InvalidParameterDefinition(
                    htype.generate_exception_message("ListsRKD.__init__", 10007)
                )

        values_types = []
        self.__values_subtypes = None

        # Either is_subtyped==False or
        # is_subtyped==[False, False, ..., False]
        if fIgnoreValuesSubTypes:
        ...
        super().__init__(potential_keys, is_typed=True, values_types=values_types)
        return
    ...
    def __setitem__(self, __key, __value, append=False):
        ...
        except ValueError:  # __key was not found in self.PotentialKeys
            return

        if not isinstance(__value, self.ValuesTypes[idx]):  # AC2
            raise cuex.InvalidParameterDefinition(
                htype.generate_exception_message("ListsRKD.__setitem__", 20001)
            )
        ...
        if fPass:
            if __key in self.keys() and append:
                ...
                # self.__getitem__(__key) must be
                # a list or a TypedList. extend
                # method is suitably overriden for
                # TypedList's

                value = self.__getitem__(__key).extend(__value)
