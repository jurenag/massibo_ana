...
def array_equality(a, b):
    ...
    if np.shape(a) != np.shape(b):
        raise cuex.ShapeException(
            htype.generate_exception_message("array_equality", 103)
        )
...
def find_neighbours(x_base, new_x):
    ...
    htype.check_type(
        x_base,
        np.ndarray,
        exception_message=htype.generate_exception_message("find_neighbours", 201),
    )
    if np.ndim(x_base) != 1:
        raise cuex.ShapeException(
            htype.generate_exception_message("find_neighbours", 202)
        )
    ...
    htype.check_type(
        ...
        float,
        np.float64,
        exception_message=htype.generate_exception_message("find_neighbours", 204),
    )
    ...
    while not fDone and i < len(x_base):
        ...
        if new_x < x_base[i]:
            ...
            # At this point, the values within
...
def find_nearest_neighbour(x_base, new_x):
    ...
    # If x_base contains at least one element, then warning_low
    # and warning_up cannot be True at the same time, so this
    # case is warning_low=warning_up=True. In this case, decide
    # which neighbour is actually closer to new_x

    else:
        up_distance = np.abs(new_x - x_base[i_up])
        low_distance = np.abs(new_x - x_base[i_low])
