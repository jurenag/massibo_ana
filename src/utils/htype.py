...
def generate_exception_message(issuer, code, extra_info=None):
    ...
    if type(issuer)!=str:
        ...
        raise cuex.TypeException("In function generate_exception_message(): ERR101: Invalid type.")
    
    if type(code)!=int:
        raise cuex.TypeException("In function generate_exception_message(): ERR102: Invalid type.")
    
    if extra_info is None:
        return f"In function {issuer}(): ERR{code}"
    else:
        if type(extra_info)!=str:
            raise cuex.TypeException("In function generate_exception_message(): ERR103: Invalid type.")
...
def check_type(input, *goal_type, exception_message="", verbosity=False):
    ...
    for i in range(len(goal_type)):
        ...
        if type(goal_type[i])!=type(type(0)) and type(goal_type[i])!=abc.ABCMeta:   # Cover the case of classes which derive from abc.ABC 
    ...
    for i in range(len(goal_type)):
        ...
        if verbosity:
            print("In function check_type(): ¿Is type("+str(input)+") == "+str(goal_type[i])+"?: ", end="")
